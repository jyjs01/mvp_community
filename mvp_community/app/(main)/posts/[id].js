...
import { View, ScrollView, Image, ActivityIndicator, RefreshControl, Dimensions, TextInput, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function PostDetailScreen() {
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      const rowsRaw = Array.isArray(data?.items) ? data.items : (Array.isArray(data) ? data : []);
      const rows = [...rowsRaw].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)); // 최신 → 오래된
      setComments(rows);
    } catch (e) {
  ...
  return (
    <KeyboardAvoidingView
      ...
      // 헤더 높이에 맞게 보정 (헤더가 있으면 64~88 정도; 필요시 숫자 조절)
              ...
              {comments.map((c) => (
                <View key={String(c.id)} style={{ paddingVertical: 4 }}>
                  ...
                  <Txt type="small" style={{ color: t.colors.muted, marginTop: 4 }}>
                    작성자 : {(c.authorName || c.author?.name || '익명')}  |  {formatDate(c.createdAt)}
                  </Txt>
                </View>
              ))}
            </View>
